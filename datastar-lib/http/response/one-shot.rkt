...
(define extra-headers/c (or/c (hash/c string? string?) #f))
(define script-attributes/c (or/c (hash/c (or/c symbol? string?) any/c) #f))
  ...
  (define datastar-headers
    (filter values
            (list (and selector (->header "datastar-selector" selector))
                  (and mode (->header "datastar-mode" (symbol->string mode)))
                  (and namespace (->header "datastar-namespace" (symbol->string namespace)))
