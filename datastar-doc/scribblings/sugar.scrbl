...
@defproc[(data-bind [signal string?]
...
@defproc[(data-on [event string?]
                ...
                [#:case case (or/c 'camel 'kebab 'snake 'pascal #f) #f]
...
@defproc[(data-on-signal-patch-filter [#:include include (or/c string? #f) #f]
...
@examples[#:eval ev #:label #f
`(details (,(data-preserve-attrs "open") (open ""))
          (summary "Title")
          "Content")
]
...
@examples[#:eval ev #:label #f
`(details (,(data-preserve-attrs '("open" "class")) (open "") (class "custom"))
          (summary "Title")
          "Content")
]
}
...
Alias of @racket[data-preserve-attrs].
}
...
@defproc[(set-all [value string?]
                      [#:include include (or/c string? #f) #f]
                      [#:exclude exclude (or/c string? #f) #f]) string?]{
...
@defproc[(toggle-all [#:include include (or/c string? #f) #f]
                         [#:exclude exclude (or/c string? #f) #f]) string?]{
...
@defproc[(get [url string?]
              [#:content-type content-type (or/c 'json 'form #f) #f]
              [#:filter-signals-include filter-signals-include (or/c string? #f) #f]
...
(list (list @bold{Keyword}                      @bold{JS Property}            @bold{Datastar Default})
...
@defproc[(clipboard [text string?]
                        [#:base64? base64? boolean? #f]) string?]{
Returns a @link["https://data-star.dev/reference/actions#clipboard"]{@tt{@"@"clipboard}} action string. Copies text to the clipboard. When @racket[#:base64?] is @racket[#t], the text is decoded from Base64 before copying.
...
@defproc[(fit [v string?]
                  ...
                  [new-min number?]
                  [new-max number?]
                  [#:clamp? clamp? boolean? #f]
                  [#:round? round? boolean? #f]) string?]{
...
@defproc[(intl [type string?]
                   [value string?]
                   [#:options options (or/c hash? #f) #f]
                   [#:locale locale (or/c string? #f) #f]) string?]{
